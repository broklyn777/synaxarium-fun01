...
const NewsletterForm = ({ title = 'Prenumerera på nyhetsbrevet' }) => {
  ...
  const subscribe = async (e) => {
    ...
    if (error) {
      setError(true)
      setMessage('din e-postadress är ogiltig eller så har du redan prenumererat!')
      return
    }
      ...
      <form className="flex flex-col sm:flex-row" onSubmit={subscribe}>
        <div>
          ...
          <input
            ...
            id="email-input"
            name="email"
            placeholder={subscribed ? "Du blir uppdaterad!  🎉" : 'E-post'}
            ref={inputEl}
            required
            type="email"
            disabled={subscribed}
        ...
        <div className="flex w-full mt-2 rounded-md shadow-sm sm:mt-0 sm:ml-3">
          <button
            className={`py-2 sm:py-0 w-full bg-primary-500 px-4 rounded-md font-medium text-white ${subscribed ? 'cursor-default' : 'hover:bg-primary-700 dark:hover:bg-primary-400'
